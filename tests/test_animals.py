...
@pytest.mark.parametrize('list_name, list_of_words',
                         LISTS_OF_WORDS.iteritems())
def test_word_format(list_name, list_of_words):
    ...
    invalid_words = [word for word in list_of_words
                     if not all(char in valid_chars for char in word)]
    assert not invalid_words, (
        'Invalid chars found in words {} from list {}').format(
...
def test_max_length():
    ...
    overly_long_words = [word for word in mega_list
                         if len(word) > 20]
    assert not overly_long_words, (
        'Your words are too long!'
        '{}'.format(overly_long_words))
